...
<head>
    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        'milk-tea': '#f2eee5',    /* 奶茶色背景 */
                        'caramel': '#c9a68a',     /* 焦糖色主色 */
                        'caramel-dark': '#a67c52', /* 深焦糖 */
                        'cream': '#fffcf8',       /* 卡片奶油白 */
                    },
                    borderRadius: {
                        'header': '2.5rem',
    ...
    <style>
        ...
        .header-bg {
            background-image: url('https://i.imgur.com/Hu16R1g.jpeg'); /* 預設小丸子圖片，請自行替換 */
...
<div id="app">
    ...
    <div class="main-container">
        ...
        <div v-if="currentTab === 'itinerary'">
            ...
            <div class="bg-blue-50/80 rounded-xl p-4 mb-4 flex items-center justify-between border border-blue-100 shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="font-bold text-gray-700">{{ currentDayData.temp }}°C</div>
                        <div class="text-xs text-gray-500">體感 {{ currentDayData.feels_like }}°C</div>
                    </div>
                </div>
                    ...
                    <div class="p-4" @click="openGoogleMap(item.name)">
                        <div v-if="item.type === 'flight'" class="bg-blue-50 p-3 rounded-lg border border-blue-100">
                            <div class="flex justify-between items-center mb-2">
                                <span class="badge bg-blue-500 text-white text-xs px-2 py-1 rounded">航班</span>
        ...
        <div v-if="currentTab === 'info'" class="flex flex-col gap-4">
            <div class="bg-white rounded-xl shadow-soft p-5 border-l-4 border-caramel">
                <h3 class="font-bold text-lg mb-3 text-caramel-dark"><i class="fa-solid fa-calculator mr-2"></i>匯率換算</h3>
                <div class="flex items-center gap-2 mb-2">
                    <input type="number" v-model="calcJpy" class="w-full p-2 border rounded bg-gray-50" placeholder="日幣 JPY">
                    <span class="text-gray-400">=</span>
                    <div class="w-full p-2 bg-gray-100 rounded text-gray-700 font-bold">NT$ {{ (calcJpy * exchangeRate).toFixed(0) }}</div>
                </div>
                <div class="text-xs text-right text-gray-400">匯率設定: {{ exchangeRate }}</div>
            </div>

            <div class="bg-white rounded-xl shadow-soft p-5 border-l-4 border-blue-400">
                <h3 class="font-bold text-lg mb-3 text-blue-600"><i class="fa-solid fa-plane mr-2"></i>航班資訊</h3>
                <div class="mb-4 border-b pb-2">
                    <div class="text-xs text-gray-500 mb-1">去程 | 02/14 (六)</div>
                    ...
                    <div class="text-xs text-center mt-1 bg-blue-50 text-blue-800 rounded py-1">CI150 中華航空</div>
                </div>
                <div>
                    <div class="text-xs text-gray-500 mb-1">回程 | 02/21 (六)</div>
                    <div class="flex justify-between items-end">
                    ...
                    <div class="text-xs text-center mt-1 bg-blue-50 text-blue-800 rounded py-1">CI221 中華航空</div>
                </div>
            </div>

            <div class="bg-white rounded-xl shadow-soft p-5 border-l-4 border-green-500">
                <h3 class="font-bold text-lg mb-3 text-green-700"><i class="fa-solid fa-bed mr-2"></i>住宿資訊</h3>
                <div class="mb-4">
                    <h4 class="font-bold">名古屋 JR 門樓飯店</h4>
                    <p class="text-sm text-gray-600 mb-1">📞 +81-52-5662111</p>
                    <p class="text-sm text-gray-600 mb-2">📍 名古屋市中村區名站1-1-3</p>
                    <button @click="openGoogleMap('名古屋JR門樓飯店')" class="w-full py-1 bg-green-50 text-green-700 rounded text-sm">導航</button>
                </div>
                <div class="border-t pt-3">
                    <h4 class="font-bold">三井花園飯店大手町</h4>
                    <p class="text-sm text-gray-600 mb-1">📞 +81-3-32543131</p>
                    <p class="text-sm text-gray-600 mb-2">📍 東京都千代田區内神田2-1-2</p>
                    <button @click="openGoogleMap('三井花園飯店大手町')" class="w-full py-1 bg-green-50 text-green-700 rounded text-sm">導航</button>
                </div>
            </div>

             <div class="bg-red-50 rounded-xl shadow-soft p-5 border border-red-100">
                <h3 class="font-bold text-lg mb-3 text-red-600"><i class="fa-solid fa-triangle-exclamation mr-2"></i>緊急聯絡</h3>
        ...
        <div v-if="currentTab === 'expense'">
            <div class="bg-caramel text-white rounded-xl p-6 mb-4 shadow-lg text-center">
                <div class="text-sm opacity-90 mb-1">總花費 (JPY)</div>
                <div class="text-4xl font-bold">¥{{ totalExpense.toLocaleString() }}</div>
                <div class="text-xs opacity-75 mt-2">約台幣 NT$ {{ (totalExpense * exchangeRate).toFixed(0).toLocaleString() }}</div>
            </div>
            ...
            <div class="flex flex-col gap-3">
                <div v-for="(exp, index) in expenses" :key="index" class="bg-white p-4 rounded-xl shadow-sm flex justify-between items-center relative">
                    ...
                    <div class="font-bold text-gray-700">¥{{ parseInt(exp.amount).toLocaleString() }}</div>
                </div>
            </div>
    ...
    <div v-if="showAddModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-sm p-5 max-h-[90vh] overflow-y-auto">
            <h3 class="font-bold text-xl mb-4 text-center">{{ isEditing ? '編輯行程' : '新增行程' }}</h3>
            <div class="space-y-3">
                ...
                <input v-model="formItem.name" placeholder="名稱" class="w-full p-2 border rounded">
                <input v-model="formItem.openTime" placeholder="營業時間" class="w-full p-2 border rounded" v-if="['spot','food','shopping'].includes(formItem.type)">
                <input v-model="formItem.ticket" placeholder="門票資訊" class="w-full p-2 border rounded" v-if="formItem.type === 'spot'">
                <textarea v-model="formItem.note" placeholder="特色 & 備註" class="w-full p-2 border rounded h-20"></textarea>
                
                <div class="border-t pt-3 mt-2">
                    <label class="text-xs text-gray-500">前往此處的交通 (估算)</label>
                    <div class="flex gap-2 mt-1">
                        ...
                        <input v-model="formItem.transportTime" placeholder="例如: 15分鐘" class="w-full p-2 border rounded text-sm">
                    </div>
                </div>
            </div>
            <div class="flex gap-2 mt-6">
                <button v-if="isEditing" @click="deleteItem" class="flex-1 py-2 bg-red-100 text-red-500 rounded-lg">刪除</button>
                <button @click="closeModal" class="flex-1 py-2 bg-gray-200 rounded-lg">取消</button>
                <button @click="saveItem" class="flex-1 py-2 bg-caramel text-white rounded-lg">儲存</button>
            </div>
        </div>
    </div>
    ...
    <div v-if="showShoppingModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-sm p-5">
            ...
            <div class="space-y-3">
                <input v-model="shoppingForm.name" placeholder="商品名稱" class="w-full p-2 border rounded">
            ...
            <div class="flex gap-2 mt-6">
                <button @click="showShoppingModal = false" class="flex-1 py-2 bg-gray-200 rounded-lg">取消</button>
                <button @click="saveShoppingItem" class="flex-1 py-2 bg-caramel text-white rounded-lg">新增</button>
            </div>
        </div>
    </div>
    ...
    <div v-if="showExpenseModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-sm p-5">
            <h3 class="font-bold text-xl mb-4 text-center">記帳</h3>
            <div class="space-y-3">
                ...
                <input v-model="expenseForm.name" placeholder="項目名稱" class="w-full p-2 border rounded">
                <input type="number" v-model="expenseForm.amount" placeholder="金額 (日幣)" class="w-full p-2 border rounded">
                <select v-model="expenseForm.method" class="w-full p-2 border rounded bg-gray-50">
                    <option value="現金">現金</option>
                    <option value="信用卡">信用卡</option>
                    <option value="Suica">Suica/IC卡</option>
                </select>
            </div>
            <div class="flex gap-2 mt-6">
                <button @click="showExpenseModal = false" class="flex-1 py-2 bg-gray-200 rounded-lg">取消</button>
                <button @click="saveExpense" class="flex-1 py-2 bg-caramel text-white rounded-lg">儲存</button>
            </div>
        </div>
    </div>
...
<script>
    ...
    createApp({
        setup() {
            ...
            // 行程資料 (預設資料)
            const tripDays = [
                { date: '2/14', week: 'SAT', fullDate: '2026-02-14', weather: 'cloud', temp: 8, feels_like: 5, location: '名古屋', hotel: '名古屋JR門樓飯店' },
                { date: '2/15', week: 'SUN', fullDate: '2026-02-15', weather: 'sun', temp: 10, feels_like: 8, location: '名古屋', hotel: '名古屋JR門樓飯店' },
                { date: '2/16', week: 'MON', fullDate: '2026-02-16', weather: 'cloud', temp: 2, feels_like: -1, location: '吉卜力', hotel: '名古屋JR門樓飯店' },
                { date: '2/17', week: 'TUE', fullDate: '2026-02-17', weather: 'snow', temp: -3, feels_like: -8, location: '合掌村', hotel: '名古屋JR門樓飯店' },
                { date: '2/18', week: 'WED', fullDate: '2026-02-18', weather: 'sun', temp: 9, feels_like: 6, location: '東京', hotel: '三井花園飯店大手町' },
                { date: '2/19', week: 'THU', fullDate: '2026-02-19', weather: 'sun', temp: 5, feels_like: 2, location: '河口湖', hotel: '三井花園飯店大手町' },
                { date: '2/20', week: 'FRI', fullDate: '2026-02-20', weather: 'sun', temp: 10, feels_like: 8, location: '東京', hotel: '三井花園飯店大手町' },
                { date: '2/21', week: 'SAT', fullDate: '2026-02-21', weather: 'cloud', temp: 11, feels_like: 9, location: '東京', hotel: '溫暖的家' }
            ];
            
            const selectedDayIndex = ref(0);
            const itineraryData = ref({}); // 結構: { '2026-02-14': [items...] }
            ...
            const initData = () => {
                if(!localStorage.getItem('itineraryData')) {
                    itineraryData.value = {
                        '2026-02-14': [
                            { id: 1, type: 'flight', name: 'CI150 抵達名古屋', time: '20:50', note: '入境預留 2.5 小時', transportType: 'train', transportTime: '50分' },
                            { id: 2, type: 'hotel', name: 'Check-in JR門樓飯店', openTime: '24hr', transportType: 'walk', transportTime: '5分' }
                        ],
                        '2026-02-15': [
                            { id: 3, type: 'spot', name: '名古屋城', openTime: '09:00-16:30', ticket: '¥500', note: '搭地鐵到名古屋城站', transportType: 'train', transportTime: '20分' },
                            { id: 4, type: 'shopping', name: '大須商店街', openTime: '11:00-20:00', note: '吃美食、逛街', transportType: 'train', transportTime: '15分' }
                        ]
                    };
                } else {
            ...
            const expenses = ref([]);
            const expenseForm = ref({ date: new Date().toISOString().split('T')[0], category: 'food', name: '', amount: '', method: '現金' });
            const totalExpense = computed(() => expenses.value.reduce((sum, item) => sum + Number(item.amount), 0));
            
            const saveExpense = () => {
                expenses.value.push({ ...expenseForm.value });
                expenseForm.value = { date: new Date().toISOString().split('T')[0], category: 'food', name: '', amount: '', method: '現金' };
            ...
            const getWeatherIcon = (type) => {
                const map = { 'sun': '☀️', 'cloud': '☁️', 'rain': '🌧️', 'snow': '❄️' };
                return map[type] || '🌤️';
            };
